...
extern char *endcode;
int get_weekday(time_t tm);

off_t friday_listdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  ...
  while(*dir) {
    ...
    int day = get_weekday((*dir)->mtime);
    if(day == 5) fprintf(outfile,"HAPPY FRIDAY!!! ");  
...
int get_weekday(time_t tm) {
      ...
      return localtime(&tm)->tm_wday; // Sunday=0, Monday=1, etc.
    
  }
 

off_t friday_rlistdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  struct _info **dir;
  off_t size = 0;
  char *err;
...
void fridayr_listdir(struct _info **dir, char *d, int *dt, int *ft, u_long lev)
{
  char *path;
  long pathsize = 0;
